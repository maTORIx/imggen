...
def _clip_encoder(text_encoder):
    """The CLIP transformer stack, across transformers versions.

    [imggen] Compatibility shim: transformers 4.x nests the encoder under
    ``text_encoder.text_model.encoder``; transformers 5.x flattened it onto
    ``text_encoder.encoder`` (the ``text_model`` wrapper is gone). Returns the
    module that holds ``.layers`` in either layout, so the SD1.5 clip-skip path
    below works on both. NOT part of upstream sd_embed.
    """
...
def get_weighted_text_embeddings_sd15(
    ...
    """
    _clip_enc = _clip_encoder(pipe.text_encoder)  # [imggen] transformers 4.x/5.x compat
    original_clip_layers = _clip_enc.layers
    if clip_skip > 0:
        _clip_enc.layers = original_clip_layers[:-clip_skip]
    ...
    # recover clip layers
    if clip_skip > 0:
        _clip_enc.layers = original_clip_layers
